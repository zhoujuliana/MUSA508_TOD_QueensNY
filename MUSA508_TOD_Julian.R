...
ring_buffers <- multipleRingBuffer(st_union(buffer), 15840, 1320)

ggplot() + geom_sf(data = ring_buffers, aes(fill = distance))

allTracts.rings <-
  st_join(st_centroid(dplyr::select(allTracts, GEOID, year)),
          multipleRingBuffer(st_union(MTAStops), 47520, 2640)) %>%
  st_drop_geometry() %>%
...
QNScrimedatFilter <- filter(QNScrimedat, year == "2016")
...
ggplot(allTracts.group)+
  ...
  labs(title = "Crime and Access to Transit", subtitle="Total Criminal Instance since 2006") +
  mapTheme()

ggplot(allTracts.group)+
  geom_sf(data=QNScrimedatFilter, 
